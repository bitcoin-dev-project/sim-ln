use std::{collections::HashMap, fs, ops::AsyncFn, path::PathBuf, sync::Arc};
...
use simln_lib::{
    cln::ClnNode, lnd::LndNode, serializers, ActivityDefinition, Amount, Interval, LightningError,
    LightningNode, NodeConnection, NodeId, NodeInfo, Simulation, SimulationCfg, WriteResults,
};
...
pub async fn create_simulation(cli: &Cli) -> Result<Simulation, anyhow::Error> {
    ...
    let SimParams { nodes, activity } =
        serde_json::from_str(&std::fs::read_to_string(sim_path)?)
            .map_err(|e| anyhow!("Could not deserialize node connection data or activity description from simulation file (line {}, col {}). {}", e.line(), e.column(), e))?;
    ...
    let sim = Simulation::new(cfg, clients, validated_activities);
