...
use self::defined_activity::DefinedActivityGenerator;
...
struct ExecutorKit {
    ...
    network_generator: Arc<Mutex<dyn NetworkGenerator + Send>>,
    ...
    async fn activity_executors(&self) -> Result<Vec<ExecutorKit>, SimulationError> {
        ...
        if !self.activity.is_empty() {
            for description in self.activity.iter() {
                let activity_generator = DefinedActivityGenerator::new(
...
async fn produce_events<N: NetworkGenerator + ?Sized, A: PayGenDisplay + ?Sized>(
    source: NodeInfo,
